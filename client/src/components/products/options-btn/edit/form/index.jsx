...
const Form = (props) => {
  ...
  return (
    <div className="create grid grid-cols-2 gap-x-4 gap-y-5 m-2 pt-6 container">
      ...
      />
      <h1 className="col-span-2">تعديل المنتج</h1>
          ...
          <div className="categories flex justify-start gap-2 flex-wrap">
            {categories.map((item, i) => (
              <button
                ...
                onClick={() =>
                  setData((prev) => ({ ...prev, categoryId: item._id }))
                }
              >
                {item.name}
              </button>
      ...
      <button
        ...
        onClick={() => {
          submit(data, files, id).then((response) => {
            setProducts((prev) => {
